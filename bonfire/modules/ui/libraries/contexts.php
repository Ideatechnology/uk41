...
class Contexts
{
	 ...
	 * @var string
	 */
	protected static $outer_id	= "side-menu";
	 ...
	 * @var string
	 */
	protected static $submenu_class	= 'nav nav-third-level';
	 ...
	 * @var string
	 */
	protected static $child_class	= 'nav nav-second-level';
	...
	public static function render_menu($mode='text', $order_by='normal', $top_level_only = FALSE)
	{
		...
		<li><a href="'.site_url().'admin/content/"><i class="fa fa-th fa-fw"></i> Dashboard</a></li>
		...
		*/
		foreach ($contexts as $context)
		{
			if ( has_permission('Site.'. ucfirst($context) .'.View') == true || permission_exists('Site.'. ucfirst($context) .'.View') == false)
			{
				...
				$nav .= "<li class='" . self::$parent_class . " {$class}'> <a href='{$url}' id='{$id}'  title='{$title}' > <i class='fa fa-th fa-fw'></i> ";
				...
				$nav .= "<span class='fa arrow'></span></a>";
	 ...
	 */
	public static function context_nav($context=NULL, $class='nav nav-second-level', $ignore_ul=FALSE)
	{
	...
	public static function build_sub_menu($context, $ignore_ul=FALSE)
	{
		...
		foreach (self::$menu as $topic_name => $topic)
		{
			...
			if (count($topic) > 1)
			{
				$list .= '<li class="' . self::$submenu_class . '">  <a href="#" > <i class="fa fa-chevron-right fa-fw"></i> '. ucwords($topic_name) .' </a>';
				$list .= '<ul class="' . self::$child_class .'">';
	...
	private static function build_item($module, $title, $display_name, $context, $menu_view='')
	{
		if (empty($menu_view))
		{
			...
			$item .= ' title="'. $title .'"> <i class="fa fa-chevron-right fa-fw"></i> '. ucwords(str_replace('_', '', $display_name)) ." </a>\n";
		...
		{
			$in = $module == self::$ci->uri->segment(3) ? 'in' : '';
			$item = '<li class="' . self::$submenu_class . '"><a href="#" > <i class="fa fa-chevron-right fa-fw"></i> '. ucwords(str_replace('_', '', $display_name)) .' <span class="fa arrow"></span></a>';
			$item .= '<ul class="' . self::$child_class .' nav nav-third-level '.$in.'">';
