...
class Users extends Front_Controller
{
		...
		if ($this->auth->is_logged_in() === FALSE)
		{
			...
			Template::set('page_title', 'Login');
                        ...
                        if($this->agent->is_mobile()){
                                Template::set_view('users/users/login_mobile');
	...
	public function changepassword(){
			...
			if (isset($_POST['set_password']))
			{
				...
				if ($this->form_validation->run() !== FALSE)
				{
					...
					if ($this->user_model->update($this->input->post('user_id'), $data))
					{
						...
						Template::redirect('/users/changepassword');
					}
					else
					{
		...
		Template::set_view('users/users/changepassword');
                        if($this->agent->is_mobile()){
                               Template::render("ajax");
                        }else{
	...
	public function profile()
	{
		...
		if (isset($_POST['save']))
		{
			...
			if ($this->save_user($user_id, $meta_fields))
			{
				...
				Template::redirect('/users/profile/sunting');
			}
			else
			{
		...
		$user2 = $this->user_model->find($this->current_user->id);
                ...
                $universitas = $this->db->query("select * from bf_universitas where "
                        . "id_universitas='".$user->universitas."'")->row();
                
                Template::set("universitas",$universitas);
		...
		Template::set('languages', unserialize($this->settings_lib->item('site.languages')));
                ...
                //load hasil ujian 
                $this->load->helper("ujian/tanggal_indo");
                $this->load->helper("ujian/formatUang");
                $this->load->model("ujian/hasil_ujian_model");
                $this->load->model("ujian/ujian_model");
                Template::set("ikuti_ujain",$this->hasil_ujian_model->countIkutiUjian($this->current_user->id));
                Template::set("list_ukuti_ujian",$this->hasil_ujian_model->ikutiUjian($this->current_user->id));
                
		
		if($this->uri->segment(3)=="sunting"){
		  if($this->agent->is_mobile()){
                                Template::set_view('users/users/sunting_profile_mobile');
                               Template::render("ajax");
                        }else{
                    Template::set_view('users/users/sunting_profile');
                    Template::render();
                        }
                }else{
				...
				if ($user_id = $this->user_model->insert($data))
				{
					...
					switch ($activation_method)
					{
						case 0:
							...
							$email_mess 	= $this->load->view('_emails/activated', array('title'=>$site_title,'link' => site_url(),"password"=>$this->input->post('password'),"user"=>$this->input->post('email')), true);
